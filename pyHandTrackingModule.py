...
class handDetector:
    def __init__(self, mode=False, maxHands=1, model_complexity=1, detectionCon=0.5, trackCon=0.5):
        ...
        self.hands = self.mpHands.Hands(int(self.mode), int(self.maxHands), int(self.detectionCon), int(self.trackCon))
    ...
    def findPosition(self, img, handNo=0, draw=True):
        ...
        if self.results.multi_hand_landmarks:
            ...
            for id, lm in enumerate(myHand.landmark):
                ...
                cx, cy = int(lm.x * w), int(lm.y * h)  # cx, cy Center Coordinates
                # print(id, cx, cy)
                xList.append(cx)
                yList.append(cy)
            ...
            if draw:
                cv2.rectangle(img, (xmin - 50, ymin - 20), (xmax + 20, ymax + 20), (0, 0, 0), 2)
